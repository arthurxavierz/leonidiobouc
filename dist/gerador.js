...
(function () {
  ...
  function desenharMarca(ctx, x, y, altura, largura, cor) {
    if (!marcaCaminhos) { return; }
    var escala = altura / MARCA.altura;
    ctx.save();
    ctx.translate(x + largura / 2, y + altura / 2);
    if (MARCA.rotacao) { ctx.rotate(MARCA.rotacao); }
    ctx.scale(escala, escala);
    ctx.translate(-MARCA.largura / 2, -MARCA.altura / 2);
    marcaCaminhos.forEach(function (parte) {
      ctx.fillStyle = cor || parte.cor;
  ...
  function desenharPlaca(ctx, m, moldura, centroX, topo) {
    ...
    if (m.larguraMarca) {
      desenharMarca(ctx, inicio, meioLinha - m.alturaMarca / 2, m.alturaMarca, m.larguraMarca, p.marcaCor);
  ...
  function prepararFontes() {
    ...
    return Promise.all([
      document.fonts.load((FONTE.displayEstilo || '400') + ' 100px ' + FONTE.display),
